...
export const FRAMES = {
  pixel9:           { w: 1620, h: 3136, inset: { t: 142, r: 170, b: 138, l: 170 }, ext: 'png' },
  'galaxy-s24':     { w: 1480, h: 2800, inset: { t: 200, r: 200, b: 200, l: 200 }, ext: 'png' },
  'iphone16-pro':   { w: 1406, h: 2822, inset: { t: 100, r: 98,  b: 100, l: 102 }, ext: 'png' },
  'iphone16-pro-max': { w: 1520, h: 3068, inset: { t: 100, r: 100, b: 100, l: 100 }, ext: 'png' },
  'iphone13-pro':   { w: 1570, h: 2932, inset: { t: 155, r: 160, b: 155, l: 160 }, ext: 'png' },
  'iphone13-pro-max': { w: 1684, h: 3178, inset: { t: 155, r: 160, b: 160, l: 160 }, ext: 'png' },
  'ipad-pro-13':    { w: 2448, h: 3132, inset: { t: 115, r: 115, b: 115, l: 115 }, ext: 'png' },
  'ipad-pro':       { w: 2068, h: 2788, inset: { t: 116, r: 116, b: 116, l: 116 }, ext: 'png' },
  'ipad-air-2020':  { w: 1940, h: 2660, inset: { t: 55,  r: 55,  b: 55,  l: 55  }, ext: 'png' },
};
...
function resolveDeviceGeometry(frameId, canvasW, canvasH, scale, position, marginTop) {
  ...
  const frameW = meta.w * scale;
  const frameH = meta.h * scale;
  const x = Math.round((canvasW - frameW) / 2);
  const y = resolvePosition(position, canvasW, canvasH, frameH, marginTop);
  return { x, y, w: Math.round(frameW), h: Math.round(frameH), scale, frameId };
}
    ...
    for (let i = 0; i < tpl.slides.length; i++) {
      ...
      for (const layer of slide.layers) {
        ...
        if (o.type === 'background') {
          bgColor = over.color || o.color || '#FFFFFF';
          ...
          composables.push({
            ...
            color: resolveColor(o.color, palette),
            align: o.align || (o.position === 'top' ? 'center' : 'center'),
          ...
          const frameId = over.frame || o.frame || deviceDefault.frame || 'pixel9';
          const scale = over.scale || o.scale || 0.58;
          const coverage = over.widthFraction || deviceDefault.widthFraction;
          const actualScale = coverage
            ? Math.min((canvasW * coverage) / (FRAMES[frameId]?.w || 1620), 1)
            : scale;
          composables.push({
            type: 'device',
            ...resolveDeviceGeometry(frameId, canvasW, canvasH, actualScale, o.position || deviceDefault.position || 'center', over.marginTop ?? o.marginTop),
            frameId,
  ...
  if (tpl.layers && Array.isArray(tpl.layers)) {
    ...
    for (const layer of tpl.layers) {
      ...
      if (o.type === 'background') {
        ...
        const frameId = over.frame || o.frame || 'pixel9';
        const scale = over.scale || o.scale || 0.58;
        composables.push({
          type: 'device',
